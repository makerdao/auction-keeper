...
from tests.conftest import keeper_address, mcd, our_address, reserve_dai, web3
...
class TestVatDaiTarget(TestVatDai):
    def create_keeper(self, dai: float):
        ...
        keeper = AuctionKeeper(args=args(f"--eth-from {self.keeper_address} "
                                         f"--flopper {self.mcd.flop.address} "
                                         f"--cat {self.mcd.cat.address} "
                                         f"--vow {self.mcd.vow.address} "
                                         f"--dai-join {self.mcd.dai_adapter.address} "
    ...
    def test_no_change(self):
        ...
        token_balance_before = self.get_token_balance()
        vat_balance_before = self.get_vat_balance()

        # when rebalancing with the current vat amount
        self.create_keeper(float(vat_balance_before))

        # then ensure no balances changed
        assert token_balance_before == self.get_token_balance()
        assert vat_balance_before == self.get_vat_balance()
    ...
    def test_join_enough(self):
        ...
        assert token_balance_before > self.get_token_balance()
        assert self.get_vat_balance() == Wad.from_number(153)

    def test_join_not_enough(self):
        # given balances before
        assert self.get_token_balance() == Wad.from_number(84)
        assert self.get_vat_balance() == Wad.from_number(153)
        ...
        assert self.get_token_balance() == Wad(0)
        assert self.get_vat_balance() == Wad.from_number(237)
    ...
    def test_exit_some(self):
        ...
        assert self.get_token_balance() == Wad.from_number(37)
        assert self.get_vat_balance() == Wad.from_number(200)
    ...
    def test_exit_all(self):
        ...
        assert self.get_token_balance() == Wad.from_number(237)
        assert self.get_vat_balance() == Wad(0)


class TestEmptyVatOnExit(TestVatDai):
    def create_keeper(self, empty_vat_on_exit: bool):
        assert isinstance(empty_vat_on_exit, bool)
        vat_behavior = "" if empty_vat_on_exit else "--keep-dai-in-vat-on-exit"
        ...
        keeper = AuctionKeeper(args=args(f"--eth-from {self.keeper_address} "
                                         ...
                                         f"--model ./bogus-model.sh"), web3=self.web3)
        assert self.web3.eth.defaultAccount == self.keeper_address.address
        assert keeper.empty_vat_on_exit == empty_vat_on_exit
        keeper.startup()
        return keeper

    def test_do_not_empty(self):
        # given dai in the vat
        keeper = self.create_keeper(False)
        self.purchase_dai(Wad.from_number(153))
        assert self.get_token_balance() >= Wad.from_number(153)
        assert self.mcd.dai_adapter.join(self.keeper_address, Wad.from_number(153)).transact(from_address=self.keeper_address)
        # and balances before
        token_balance_before = self.get_token_balance()
        vat_balance_before = self.get_vat_balance()
    ...
    def test_empty(self):
        ...
        # when creating and shutting down the keeper
        keeper = self.create_keeper(True)
        keeper.shutdown()

        # then ensure the vat was emptied
        assert self.get_token_balance() == token_balance_before + vat_balance_before
        assert self.get_vat_balance() == Wad(0)
